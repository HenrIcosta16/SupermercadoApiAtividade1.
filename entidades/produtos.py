...
def produtos_post():
    ...
    # 3 - Executar.
    cursor.execute(
        "insert into tb_produtos(nome) values (?)", (produtosNovos['nome'],))
...
def getProdutoById(idProduto):
    try:
        ...
        # 3 - Executar.
        cursor.execute(
            "select * from tb_produtos where id = ?", (idProduto,))
...
def produtos_put(idProduto):
    try:
        ...
        if resultset is not None:
            # 3 - Executar.
            cursor.execute("update tb_produtos set nome=? where id=?",
...
def produtos_delete(idProduto):
    try:
        ...
        if resultset is not None:
            # 3 - Executar.
            cursor.execute(
                "delete from tb_produtos where id = ?", (idProduto, ))
            # 3.1 - Confirmar - commit.
            connection.commit()
