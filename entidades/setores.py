...
def setores_post():
    ...
    # 3 - Executar.
    cursor.execute(
        "insert into tb_setores(nome) values (?)", (setoresNovos['nome'],))
...
def getSetorById(idSetores):
    try:
        ...
        # 3 - Executar.
        cursor.execute(
            "select * from tb_setores where id = ?", (idSetores,))
...
def setores_put(idSetores):
    try:
        ...
        if resultset is not None:
            # 3 - Executar.
            cursor.execute("update tb_setores set nome=? where id=?",
...
def setores_delete(idSetores):
    try:
        ...
        if resultset is not None:
            # 3 - Executar.
            cursor.execute(
                "delete from tb_setores where id = ?", (idSetores, ))
            # 3.1 - Confirmar - commit.
            connection.commit()
