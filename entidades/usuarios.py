...
def usuarios_post():
    ...
    # 3 - Executar.
    cursor.execute(
        "insert into tb_usuarios(nome) values (?)", (usuariosNovos['nome'],))
...
def getUsuariosById(idUsuarios):
    try:
        ...
        # 3 - Executar.
        cursor.execute(
            "select * from tb_usuarios where id = ?", (idUsuarios,))
...
def usuarios_put(idUsuarios):
    try:
        ...
        if resultset is not None:
            # 3 - Executar.
            cursor.execute("update tb_usuarios set nome=? where id=?",
...
def usuarios_delete(idUsuarios):
    try:
        ...
        if resultset is not None:
            # 3 - Executar.
            cursor.execute(
                "delete from tb_usuarios where id = ?", (idUsuarios, ))
            # 3.1 - Confirmar - commit.
            connection.commit()
